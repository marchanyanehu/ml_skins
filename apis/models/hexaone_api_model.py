...
class HexaOneApiModel(APIClient):
    ...
    async def async_get(self, endpoint, params=None, headers=None):
        url = self._build_url(endpoint, params)
        async with httpx.AsyncClient() as client:
            print(f"Requesting {url}")
            response = await client.get(url, timeout=100, headers=headers)
        return self._handle_response(response)

    def get_account_info(self):
        return self.async_get("api/account", headers=self._build_auth_header(self.api_key))
